(() => {
    ...
    const drops = document.querySelectorAll('.task17_imgWrapper_img')
    const answersWrapper = document.querySelector('.task17_answers')
    ...
    function setAnswers() {
        answers.sort(() => Math.random() - 0.5).forEach(item => {
            ...
            answer.classList.add('task17_answer')
    ...
    document.addEventListener('mouseup', (e) => {
        if (draggingItem) {
            ...
            if (elemBelow.classList.contains("task17_imgWrapper_img") && elemBelow.children.length === 0) {
