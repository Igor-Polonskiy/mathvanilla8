(() => {
    const reloadTaskBtn = document.querySelector('.task22_reloadTask')
    const checkingTaskBtn = document.querySelector('.task22_checkingTask')
    const checkTask = document.querySelector('.task22_checkTask')
    const chek_answerTxt = document.querySelector('.task22_chek_answer')
    const drops = document.querySelectorAll('.task22_imgWrapper_img')
    const answersWrapper = document.querySelector('.task22_answers')
    const task = document.querySelector('.task22_wrapper')
    ...
    function setAnswers() {
        answers.sort(() => Math.random() - 0.5).forEach(item => {
            ...
            answer.classList.add('task22_answer')
    ...
    task.addEventListener('mousedown', (e) => {
        if (e.target.classList.contains('task22_answer')) {
    ...
    document.addEventListener('mouseup', (e) => {
        if (draggingItem) {
            ...
            if (elemBelow.classList.contains("task22_imgWrapper_img") && elemBelow.children.length === 0) {
